...
def deploy_dynamics(method, all_params_and_data, standard=True):
    ...
    if method == "D-SGLD":
        ...
        for iter in tqdm(range(num_iterations)):
            ...
            # # Plot samples at start/end of deployment
            # if iter == 0:
            #     plt.subplot(1,2,1)                       
            #     for aa in range(num_agents):
            #         colors = ['b', 'g', 'r', 'm', 'k']
            ...
            #     plt.title("Initial Distribution of Samples")
            ...
            #     plt.xlabel('$\mathregular{x_1}$')
            #     plt.ylabel('$\mathregular{x_2}$')
            #     plt.title("Final Distribution of Samples")
...
expers = [True]
expers += [False for _ in range(9)]
for _ in range(10):
    wasserstein_dists = deploy_dynamics("D-ADMMS", all_params_and_data, standard=expers[_])
    results['D-ADMMS'] = wasserstein_dists
    for aa in range(agent_samples):
        if _ == 0:
            plt.plot(range(len(wasserstein_dists[aa])), wasserstein_dists[aa], label="standard", color="blue")
        elif _ == 1:
            plt.plot(range(len(wasserstein_dists[aa])), wasserstein_dists[aa], label="random", color="orange")
        else:
            plt.plot(range(len(wasserstein_dists[aa])), wasserstein_dists[aa], color="orange")
